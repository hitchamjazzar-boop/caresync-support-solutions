...
export default function Memos() {
  ...
  useEffect(() => {
    applyFilters();
  }, [memos, sentMemos, searchQuery, filterType, filterStatus, activeTab]);
  ...
  const fetchSentMemos = async () => {
    ...
    if (memosData) {
      ...
      setSentMemos(memosWithDetails);
    }
  };

  const applyFilters = () => {
    const source = activeTab === 'received' ? memos : sentMemos;
    let filtered = [...source];
  ...
  const renderMemoCard = (memo: Memo, isSent: boolean) => (
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <CardDescription className="mt-1 text-xs sm:text-sm">
                {isSent ? (
                  <span className="block sm:inline">To: {memo.recipient_profile?.full_name || 'Unknown'}</span>
                ) : (
