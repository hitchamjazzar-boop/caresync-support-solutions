...
export function BirthdayAnnouncementDialog({ open, onOpenChange, onSuccess }: BirthdayAnnouncementDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const defaultMessage = message.trim() || `Wishing ${namesText} a wonderful birthday filled with joy and happiness! 🎉`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Select employees and add a special birthday message
          </DialogDescription>
        </DialogHeader>
